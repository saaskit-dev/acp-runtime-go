...
- ACP adapter: `npm exec --yes @agentclientprotocol/codex-acp --` (unpinned, resolves latest; migrated from the deprecated `@zed-industries/codex-acp@0.15.0` which was renamed. Latest observed at time of writing: 1.1.0)
...
- `CreateCodexAgent` must start the maintained ACP wrapper, not `codex acp`. The Go runtime now uses `npm exec --yes @agentclientprotocol/codex-acp --` (unpinned, resolves latest).
