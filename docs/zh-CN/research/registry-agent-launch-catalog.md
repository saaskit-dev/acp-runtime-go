...
| `claude-acp` | Claude Agent | npx | `npm exec --yes @agentclientprotocol/claude-agent-acp --`（unpinned，自动拉取 latest） |
| `cline` | Cline | npx | `npx cline@2.11.0 --acp` |
| `codebuddy-code` | Codebuddy Code | npx | `npx @tencent-ai/codebuddy-code@2.70.1 --acp` |
| `codex-acp` | Codex CLI | binary / npx | `npm exec --yes @agentclientprotocol/codex-acp --`（unpinned，自动拉取 latest）或下载后执行 `./codex-acp` |
