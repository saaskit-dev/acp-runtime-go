...
- ACP adapter：`npm exec --yes @agentclientprotocol/codex-acp --`（unpinned，自动拉取 latest；从已废弃的 `@zed-industries/codex-acp@0.15.0` 迁移，旧包已改名。撰写时 latest 为 1.1.0）
...
- `CreateCodexAgent` 必须启动维护中的 ACP wrapper，而不是 `codex acp`。Go runtime 现在使用 `npm exec --yes @agentclientprotocol/codex-acp --`（unpinned，自动拉取 latest）。
